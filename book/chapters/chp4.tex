...
\noindent The \pythoninline{Domain} object reads the entries,
registered above as \pythoninline{smap.add("10",1)} etc, of the
\pythoninline{SubdomainMap} with respect to the ordering of the
entries in our \pythoninline{surfaces} list. The important point here
is that the order of the entries in the \pythoninline{surfaces} list
plays a key role; permuting the entries will yield different results.
For the code above, the string \pythoninline{"10"} is interpreted as
...
of only the gray matter. Similarly, the string \pythoninline{"11"} is
...
within both surfaces. We will discuss \pythoninline{SubdomainMap}
...
\pythoninline{SubdomainMap}, let's consider another, more involved
...
\begin{figure}[t]
  ...
  \caption{The upper left panel, (A), shows three colored squares that
    ...
    files, in the \pythoninline{surfaces} list, at their simplest
    ...
    separately tagged using \pythoninline{smap.add} and the surfaces
    defined in the \pythoninline{surfaces} list. The regions of (B)
    are colored by an independent color scheme that shows all possible
    combinations of volumes addressable by \pythoninline{smap.add}.
...
list \pythoninline{surfaces}. The ventricles in the left hemisphere
...
\pythoninline{SubdomainMap} object, such as \pythoninline{smap}, or by
using the member function \pythoninline{set\_number\_of\_surfaces} of
the \pythoninline{SubdomainMap} class (for example,
\pythoninline{smap.set\_number\_of\_surfaces}). Therefore, as an
...
The segmentations can be inspected using a tool such as Freeview. As an
...
\begin{figure}
  ...
  \caption{Post-processed ventricle surface extracted from MRI using
    \freesurfer{}. This surface file (\emp{ernie-ventricles.stl}) is
    created by the script \emp{mri2fem/chp4/extract-ventricles.sh}.}
...
\item
  ...
  \pythoninline{union\_partially\_overlapping\_surfaces} offers an
...
included in \emp{mri2fem/chp4/fullbrain-five-domain.py}). We wrap the
...
\index{parcellations}
{\freesurfer}'s \emp{recon-all} automatic segmentation process can
identify almost two hundred different brain
regions.\footnote{{\freesurfer}'s \emp{recon-all} parcellates a brain
...
of these regions using \emp{freesurfer/ernie/mri/wmparz.mgz} as an
example.
\begin{figure}
\begin{center}
  \hspace{2em}
...
\index{FEniCS!\emp{MeshFunction}} We aim to map the parcellation
...
\emp{wmparc.gz}; this parcellation was created from a set of T1 MR
images. We will also work with a mesh file:
\emp{ernie-brain-32.xdmf}. This mesh file was generated from surfaces
...
mesh elements is to use a {\fenics} \emp{MeshFunction}. Mesh functions
