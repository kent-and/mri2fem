...
in Figure~\ref{fig:chp4:ernie-tagged-twodomain-mesh}.  To see the view of 
Figure~\ref{fig:chp4:ernie-tagged-twodomain-mesh} in ParaView, first load 
\emp{ernie-gw.vtu} by selecting \button{File$\rightarrow$Open} and selecting 
\emp{ernie-gw.vtu}, then click \button{Apply} to show the mesh.  In the 
left-hand window, under the \emp{Coloring} heading, select the second 
...
\pythoninline{smap.set\_number\_of\_surfaces}. Therefore, as an alternative, one
...
select all voxels from the fourth ventricle, we can use \emp{-{}-match
...
\begin{figure}
  ...
  \includegraphics[width=0.49\textwidth]{./graphics/chp4/ernie-final-comp-d}
	...
	%removed (left) and ventricles not removed but marked in black (right).
	}
...
to a more complicated tagging process with \emp{SubdomainMap} but
...
T1 images. FEniCS offers the concept of a \emp{MeshFunction} to
...
the corresponding image data values into the \emp{regions} mesh function:
...
Let us assume that, in Python, we have a FEniCS \emp{mesh} with FEniCS
