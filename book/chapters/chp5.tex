...
computational mesh.  
%display sharp transitions that will need to be
...
extracted indices\footnote{Voxels are located based on the DOF coordinates in 
the \emp{FunctionSpace} object.  This approach guarantees that there are no 
missing values as every DOF maps to some voxel.  However, some voxels may 
correspond to more than one DOF as there may be more tetrahedron in the mesh 
than there are voxels if the mesh has a resolution which is less than the 
1mm resolution of the T1 (voxel) image space.} (now in voxel space):
