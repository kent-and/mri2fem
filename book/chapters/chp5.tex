...
\button{Tools$\rightarrow$Manage Plugins}, selecting the plugin option labeled
\button{AnalyzeNifTIIO} and then clicking \button{Load Selected}. You
...
\noindent Try opening the files \emp{adc.nii} and
\emp{fa.nii}; the results should look similar to 
...
The \emp{valid} mask is then reshaped to fit the dimensions of the original 
\emp{mask}, created from the \emp{mask\_file}, and the number of zeros, 
...
The function \pythoninline{find_valid_adjacent_tensor}, defined in 
...
use the code located in \emp{mri2fem/dti\_data\_to\_mesh.py} to accomplish this 
...
freedom associated with the DG \emp{TensorFunctionSpace} defined on our
...
to do this using registration tools such as \emp{bbregister}, which was used with
...
the inverse of the augmented matrix found in \emp{register.lta}
located in folder \emp{\$SUBJECTS\_DIR/ernie/dti}. The augmented
