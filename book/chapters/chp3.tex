\chapter{Getting started: from T1 images to simulation}
\label{chp:chp3}
...
e.g., in~\emp{\ernieT1}, we can pick \emp{IM\_0162}. Next, we
...
\emp{recon-all}:\footnote{It is advisable to do this on a separate
  core or overnight.}
...
\terminal{\$ mris\_convert ./lh.pial lh.pial.stl}
...
(\pythoninline{meshfile}), and an optional mesh resolution,\footnote{
  The \pythoninline{resolution} parameter is a number that coarsely
  specifies the size of the mesh elements; higher values of
  \pythoninline{resolution} produce meshes composed of smaller
  average tetrahedral diameters. The \pythoninline{resolution}
  parameter default in this script is 16. Small changes in this value
  will not typically produce a visual difference
  (i.e.~\pythoninline{resolution}=18) while larger differences
  (e.g.~\pythoninline{resolution}=32 or 64) will produce visually
  distinct meshes with a clear difference in tetrahedral element
  size.} the script creates an \svmtk{} \pythoninline{Domain} object,
generates a volume mesh from the surface via the call to
\pythoninline{create\_mesh}, and saves this mesh in the
\pythoninline{.mesh} format to the output mesh file. The mesh
...
\pythoninline{n}; we mention that \pythoninline{n} should not be
...
the other hand, the floating point value of \pythoninline{L}
corresponds to a quantitative mesh parameter: \pythoninline{L}
...
(\pythoninline{True}) or a Laplacian smoothing (\pythoninline{False})
...
\pythoninline{n} sets the number of times the smoothing process should
...
bumps). Finally, the floating point value \pythoninline{eps}
...
\begin{figure}
  ...
  \caption{Surface smoothing: original pial surface 
    (\emp{lh.pial.remesh.stl}, top left), after Taubin smoothing with
    {\svmtk} (\emp{lh.pial.smooth.stl}, top middle), and Laplacian over-smoothing 
...
(\pythoninline{tau}) of three min. Note that we use the Constant type
to represent \pythoninline{time}; it is useful for updating functions
...
\pythoninline{time} with the timestep~\pythoninline{tau}, assemble the
right-hand side into the vector \pythoninline{b}, apply the Dirichlet
...
We can compute the total amount of solute by integrating the
...
\newpythonsnippet{chp3}{diffusion.py}{87}{96} It can also be practical
...
scripts (\emp{mri2fem/chp3/diffusion.py}). The concentration at
...
\begin{figure}
    ...
    \end{tabular}
    \caption{Plots of the quantities of interest, in arbitrary units
      (AU), associated with the computed concentration: total amount
      of solute $Q$ over time $t$ (left) and concentration $u(p, t)$
      for a given point $p$ over time $t$ (right). The unphysiological
      early concentration (right) results from a numerical artifact
      that is common when solving diffusion problems; in
      Chapter~\ref{chp:chp6}, we discuss a way to adjust our solution
...
\index{mass lumping}
%
The plot of total solute (Figure~\ref{fig:chp3:png-plots}, left) shows
a steady increase of solute within the brain as the available tracer
at the boundary (implemented by the concentration boundary condition
\pythoninline{u\_d}) increases. As the process is diffusive, the total
solute will increase until the concentration of the solution within
the brain and at the boundary are in equilibrium. The approximation of
the solute concentration (Figure~\ref{fig:chp3:png-plots}, right) at a
given point over time demonstrates an interesting numerical artifact:
the concentration drops below zero to become negative between 0 and
550 min. Negative solute concentrations, which are clearly
unphysiological, are a common numerical problem in diffusion
simulations. A partial and often used remedy that can be applied
within the context of diffusion simulations in the
brain~\cite{croci2019uncertainty}, is \emph{mass lumping}. Mass
lumping can reduce spurious negative concentrations, but may worsen
the overall convergence of the numerical solutions. We will return to
this issue in Chapter~\ref{chp:chp6}.

\subsection{Visualization of solution fields}
\index{ParaView} To visualize computed solution fields, for instance,
...
\button{File$\rightarrow$Open} and selecting the .pvd or .vtu file(s)
...
command \emp{mri\_probedicom} can be used to compare the metadata of
different DICOM files with the flag \emp{-{}-compare} followed by two
...
%
%\noindent To probe the book DICOM dataset for the tag \textit{Protocol Name}, which
...
(also available at \emp{mri2fem/chp3/extract\_dicom.sh}). The script
...
the flag \emp{-help}, which will print detail about the
entire process and provide references to related articles and texts.

The command \emp{recon-all} is a sequential process that consists of
...
\emp{recon-all} flags are shown below; a full list of options is
...
\emp{-autorecon3}, as shown above. This approach can also be useful
...
correct the error, and run \emp{recon-all} with the flag
\emp{-autorecon2-pial}. Specific details for the
\emp{-autorecon2-pial} flag, which we do not discuss here, can be
