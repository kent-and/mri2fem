...
\chapter{Getting started: from T1 images to a basic simulation}
\label{chp:chp3}
...
an injected tracer\footnote{As we mentioned, MRI scanners work by manipulating 
a magnetic field around the patient and then measuring the body's molecular 
interaction with radio waves. Non-toxic substances, designed specifically 
with this interaction in mind, have been developed that can injected into 
a patient to improve the visibility of internal body structures.  These 
substances are referred to as \textit{MRI tracers} or, also, as 
\textit{MRI contrast agents}.  Dennis Carr and Wolfgang Schorner published 
the first tracer-enhanced MR images, in humans, using gadolinium 
...
the \pythoninline{resolution} parameter is a number that coarsely specifies 
the size of the mesh elements; higher values of \pythoninline{resolution} 
produce meshes comprised of smaller average tetrahedral diameters.  The 
\pythoninline{resolution} parameter default is 16.  Small changes in this 
value will not typically produce a visual difference 
(i.e.~\pythoninline{resolution}=18) though large differences 
(e.g.~\pythoninline{resolution}=32 or 64) will produce visually distinct 
meshes with a clear difference in tetrahedral element size.}, the script 
creates an \svmtk{} \pythoninline{Domain} object, generates a volume mesh from the
...
\begin{figure}
  ...
  \caption{Original (left) pial surface and (right) after remeshing with 
  \svmtk{}.  Remeshing improves the fidelity of the mesh and, at a high level, 
  starts by subdividing tetrahedron based on a criteria, such as a maximal 
  diameter or anisotropy condition, and further subdividing neighboring 
  tetrahedron as needed (e.g.~to ensure overall mesh quality).}
...
recommend Taubin smoothing as the Laplacian approach tends to significantly 
diminish anatomical features. The integer value \pythoninline{n} sets the number 
of times the smoothing process should take place. Higher values will produce a 
smoother mesh; however, too high of a value may result in a loss of resolution 
in brain features, such as the sulci and gyri (grooves and bumps), on the brain 
surface. Finally, the floating point value \pythoninline{eps} determines the 
strength of the smoothing operation for each smoothing iteration, and should 
be in the interval $[0, 1]$ with $0.0$ ($1.0$) indicating no (full)
smoothing.
\begin{figure}
  \centering
...
%\begin{figure}
%  \centering
%  \includegraphics[width=0.80\textwidth]{./graphics/chp3/smooth_zoom.png}
%  \caption{Surface smoothing: original pial surface 
...
%  \label{fig:chp3:ernie-smoothing-zoomed}
%\end{figure}
...
(\emp{mri2fem/chp3/diffusion.py}).  The concentration at various points%
\footnote{The point we consider, $p$, does not represent a point of any 
particular significance; $p$ was selected as an arbitrary point in the 
interior of the brain mesh.} can be investigated by modifying the value of 
$p$, in the script, as shown in the code block below:
\newpythonsnippet{chp3}{diffusion.py}{62}{68}
...
\begin{figure}
	...
	point $p$ over time $t$ (right).  The unphysical early 
        concentration (right) results from a numerical artifact which is common 
        when solving diffusion problems; we discuss an adjustment in our solution 
        method, to remedy this behavior, in Chapter~\ref{chp:chp6}. }
    \label{fig:chp3:png-plots}
\end{figure}

\index{mass lumping}
The plot of total solute (Figure~\ref{fig:chp3:png-plots}, left) shows a 
steady increase of solute within the brain as the available tracer 
...
solute will increase until the concentration of the solution within the brain 
and at the boundary are in equilibrium. The approximation of the solute 
concentration (Figure~\ref{fig:chp3:png-plots}, right) at a given point over
...
in the FreeSurfer documentation \cite{freesurfer-wiki}.  This will create new 
